...
def mentor_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        if request.user.is_authenticated and (request.user.role == 'mentor' or request.user.is_wmi_domain()):
...
def mentee_required(view_func):
    ...
    def _wrapped_view(request, *args, **kwargs):
        if request.user.is_authenticated and (request.user.role == 'mentee' or request.user.is_wmi_domain()):
            return view_func(request, *args, **kwargs)
        return HttpResponseForbidden("You do not have permission to access this page. Please request Mentorship committee for access.")
    return _wrapped_view
